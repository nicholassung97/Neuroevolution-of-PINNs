...
def loss(prediction, inputs):
    ...
    # PDE (physics laws): u_t + u*u_x - vis*u_xx = 0   
...
# Task
class nonlinear(VectorizedTask):
    """PINN for 1D transient nonlinear Burgers' equation.
    We model the regression as a one-step task, i.e., 'nonlinear.reset' returns a batch of data to the agent,
    the agent outputs predictions, `nonlinear.step` returns the reward loss and terminates the rollout."""
    def __init__(self):
        self.max_steps = 1
        # Input shape is 2
